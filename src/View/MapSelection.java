...
public class MapSelection {
    ...
    private view.MapSelectionItem[] mapSelectionItems;
    ...
    public void draw(Graphics g){
        ...
        for(view.MapSelectionItem item : mapSelectionItems){
    ...
    private view.MapSelectionItem[] createItems(ArrayList<String> maps){
        if(maps == null)
            return null;

        int defaultGridSize = 100;
        view.MapSelectionItem[] items = new view.MapSelectionItem[maps.size()];
        for (int i = 0; i < items.length; i++) {
            Point location = new Point(0, (i+1)*defaultGridSize+200);
            items[i] = new view.MapSelectionItem(maps.get(i), location);
        }
